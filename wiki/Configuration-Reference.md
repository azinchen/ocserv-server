...
| `VPN_GATEWAYS_FILE` | _(unset)_ | File defining named gateways, one `name ip` (or `name=ip`) per line with `#` comments — an alternative to a large `VPN_GATEWAYS`. Merged with the env var (file wins). Read at **startup only**; changing it needs a container restart (unlike `VPN_USER_GATEWAY_FILE`, which reloads live). |
| `VPN_GATEWAYS6_FILE` | _(unset)_ | File defining the per-gateway IPv6 addresses (`name ip6` per line). Merged with `VPN_GATEWAYS6` (file wins). |
