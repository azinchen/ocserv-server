...
| `VPN_GATEWAYS_FILE` | _(unset)_ | File defining named gateways, one `name ipv4 [ipv6]` per line with `#` comments — an alternative to `VPN_GATEWAYS` + `VPN_GATEWAYS6`. The optional 3rd column is the gateway's IPv6 address, so both families are defined in one file. Either address may be `block` to block that family for the gateway's users — its forwarded packets are rejected (ICMP admin-prohibited) so the client fails fast and falls back, with no leak (`block` in the IPv4 column makes an IPv6-only gateway). Merged with the env vars (file wins per field). Read at **startup only**; changing it needs a container restart (unlike `VPN_USER_GATEWAY_FILE`, which reloads live). See [Gateway Mode#blocking-a-family-per-gateway](Gateway-Mode#blocking-a-family-per-gateway). |
