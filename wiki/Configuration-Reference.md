...
| `VPN_BYPASS_TARGETS_FILE` | _(unset)_ | File alternative to `VPN_BYPASS_TARGETS`: one `pool target` (or `pool=target`) per line, `#` comments allowed. Merged with the env variable, the **file wins** on a conflicting pool. Read once at container start, like the pool set itself — editing it needs a restart (unlike the hot-reloadable user maps). |
