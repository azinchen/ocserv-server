...
| `MSS` | _(unset)_ | Clamp the client TCP (CSTP) MSS. Unset clamps to the **path MTU** (a no-op on plain 1500 links, so existing setups are unaffected). Set a number (e.g. `1300`) to force a hard cap when the path to clients has a smaller MTU than the local interface **and** PMTUD is broken — e.g. a macvlan reaching a VPS peer, PPPoE, or tunnelled uplinks — where full-size segments would otherwise be silently dropped and the tunnel would carry no data. See [Networking NAT and Routing#mss-clamping](Networking-NAT-and-Routing#mss-clamping). |
