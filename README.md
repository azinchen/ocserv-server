...
| `VPN_GATEWAYS_FILE` | _(unset)_ | Path to a file defining named gateways, one `name ip` (or `name=ip`) per line, `#` comments allowed — an alternative to a large `VPN_GATEWAYS`. Merged with it (file wins). Read at **startup** (unlike the user map, gateway definitions are not hot-reloaded). |
| `VPN_GATEWAYS6_FILE` | _(unset)_ | Same for the per-gateway IPv6 addresses (`name ip6` per line). Merged with `VPN_GATEWAYS6`. |
